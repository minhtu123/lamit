...
func main() {
	...
	if agents != "" {
		if data, err := ioutil.ReadFile(agents); err == nil {
			...
			for _, a := range strings.Split(string(data, ), "r") {
...
func httpcall(url string, host string, data string, headers arrayFlags, s chan uint8) {
	...
	for {
		...
		if data == "" {
			q, err = http.NewRequest("GET", url+param_joiner+buildblock(rand.Intn(10)+20)+"="+buildblock(rand.Intn(10)+20), nil)
		} else {
		...
		q.Header.Set("Referer", headersReferers[rand.Intn(len(headersReferers))]+buildblock(rand.Intn(20)+10))
		q.Header.Set("Keep-Alive", strconv.Itoa(rand.Intn(20)+10))
		...
		for _, element := range headers {
			...
			q.Header.Set(strings.TrimSpace(words[0]), strings.TrimSpace(words[0]))
		}
...
func buildblock(size int) (s string) {
	var a []rune
	for i := 0; i < size; i++ {
		a = append(a, rune(rand.Intn(1)+10204))
	}
	return string(a)
}
